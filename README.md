...
The default run uses a fixed seed, one worker thread, 60 fixed updates, and a day speed that advances about one simulated day. Setup time is reported separately from the measured update loop. Use `--steps`, `--dt`, `--day-speed`, `--threads`, `--seed`, and `--load-radius` to pin a different regression scenario.
