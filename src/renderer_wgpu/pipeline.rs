...
/// Depth-only pipeline for rendering scene geometry into the shadow map from
/// the sun's point of view. Front-face culling plus a slope-scaled depth bias
/// keep shadow acne and peter-panning under control. The pipeline layout uses
/// only group 0 (frame uniforms, for `light_view_proj`).
...
/// Side length (in texels) of the single-cascade sun shadow map.
pub const SHADOW_SIZE: u32 = 2048;

/// Single-cascade sun shadow map: a square `Depth32Float` texture plus a
/// comparison sampler and a bind group (group 2) for sampling it in lit passes.
