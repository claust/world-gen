// Thumbnail shader: a shadow-free variant of instanced.wgsl used by the offline
// thumbnail renderer (which has no shadow map / no shadow pass). Keeps the
// FrameUniform layout in sync with the rest of the renderer so the shared frame
// buffer is read correctly, but does not sample a shadow map. This file is
// included standalone (not concatenated with lighting.wgsl), so it carries its
// own copy of the hemisphere-ambient helpers.
...
// --- Hemisphere ambient (copy of lighting.wgsl, kept in sync) ---------------
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let color = input.albedo * hemisphere_ambient(n)
        + input.albedo * direct * 0.82 * material.sun_color.rgb;
