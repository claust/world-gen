...
use super::instancing::{upload_instances, upload_prototype, InstanceData};
...
impl ThumbnailRenderer {
    ...
    pub fn generate_all(
        ...
        queue.write_buffer(&self.material_bind.buffer, 0, cast_slice(&[material_data]));
        ...
        for (i, entry) in herbarium.plants.iter().enumerate() {
            let tex_id = self.render_plant(device, queue, &entry.species, i as u32, egui_renderer);
    ...
    pub fn invalidate(
        ...
        if let Some(entry) = herbarium.plants.get(index) {
            let tex_id =
                self.render_plant(device, queue, &entry.species, index as u32, egui_renderer);
            self.textures[index] = tex_id;
        } else {
    ...
    fn render_plant(
        ...
        self.frame_bind.update(queue, &frame_uniform);
        ...
        let instance_chunk = upload_instances(device, &[instance_data], "thumb-inst")?;
...

        {
            ...
            pass.set_vertex_buffer(0, prototype.vertex_buffer.slice(..));
            pass.set_index_buffer(prototype.index_buffer.slice(..), wgpu::IndexFormat::Uint32);
            pass.set_vertex_buffer(1, instance_chunk.instance_buffer.slice(..));
            pass.draw_indexed(0..prototype.index_count, 0, 0..1);
        }

        queue.submit(std::iter::once(encoder.finish()));

        // Register as egui texture
        let tex_id =
